...
| --- | --- | --- |
| `trade_id` | P4 | 板寄せ約定1件 (買方 −cash・売方 +cash の現物授受。EXCH は清算・決済のみで手数料は発生しない) |
...
| `liquidation_id` | P4/P7 | 倒産清算の残余分配・信用ポジションの強制決済 (`LIQ:NNNNNN`、[09 §強制決済](09-markets-and-trading.md)) |
...
| `POOL_SUPPLY` | P4 | 貸借プールへのアセット預入 (`loan_id`= プール持分発行の起点。供給者 → `POOL:<asset>` 移転) |
| `POOL_WITHDRAW` | P4 | 貸借プールからの持分償還・引出 (`POOL:<asset>` → 供給者の現物移転、利用可能残高の範囲) |
| `LOAN` | P4 | 信用ポジション建てのためのプール貸出 (`POOL:<asset>` → 借り手の現物移転、原因参照 `loan_id`= `POS:NNNNNN`) |
| `REPAY` | P4/P7 | 借入アセットのプールへの返済 (借り手 → `POOL:<asset>` 移転、原因参照 `loan_id`) |
| `INTEREST` | P7 | 信用ポジションの貸付利息支払 (借り手 → `POOL:<asset>` + 保険基金 `INSF:<cc>` への移転、原因参照 `loan_id`) |
| `LIQUIDATION_PENALTY` | P4 | 強制決済された名目への清算ペナルティ (借り手担保 → `INSF:<cc>` 移転、原因参照 `liquidation_id`) |
| `HAIRCUT` | P4 | 不良債権 (`equity<0`) の劣後吸収 (`INSF:<cc>` 補填 → 尽きれば供給者持分への按分ヘアカット、原因参照 `liquidation_id`) |
| `AMM_SUPPLY` | P4 | AMM プールの準備金 (`base`+`quote`) 預入と持分発行 (供給者 → `AMM:<pair>` 移転) |
| `AMM_WITHDRAW` | P4 | AMM プールの持分償還・準備金引出 (`AMM:<pair>` → 供給者の現物移転、準備金比按分) |

> いずれも実在アセットの移転 (二重仕訳・総量保存) であり、ミント/バーン集合には属さない ([00 §0.17](00-glossary.md))。`POS:NNNNNN`/`LIQ:NNNNNN` は単調増加・6桁ゼロ詰めの ID で、`loan_id`/`liquidation_id` として `LOAN`/`REPAY`/`INTEREST`/`HAIRCUT`・清算 posting の原因参照 `cause_ref` を成す。信用ポジション・貸借プール・保険基金・AMM プールの正準スキーマは [15 データモデル](15-data-model.md)、機構は [09 市場と取引](09-markets-and-trading.md) を参照。
...
- **非負残高**: いかなる posting も、適用後に `balance[entity][asset] < 0` となってはならない。現物の空売りは禁止する。信用ポジションは借入アセットを貸借プールから現物で受け取り (`LOAN`)、その返済債務を `Position` の負債 (`borrowed_value + accrued_interest`) として計上することで表現し (8.8)、現物 `COMM`/`CUR` 残高は負にしない ([09 §信用取引](09-markets-and-trading.md))。

### 8.5.1 約定の現金保存

1件の市場約定では、買方が支払う現金と売方が受け取る現金が常に一致する。取引手数料は存在せず、`EXCH` は清算・決済のみを担い現金を収受しない。`CUR` の総量は約定で変化しない (移転であってミント/バーンでない)。
...
買方支出 = 売方受取 が常に成立し、`CUR` 総量は保存される。約定価格 `p*` の決定 (出来高最大化) と注文種別は [09](09-markets-and-trading.md) に従う。
...
P4 で各取引ペアの板寄せが約定すると、約定ごとに `trade_id` を原因とする posting が記帳される。`base` 資産と `quote` 通貨の双方について借方=貸方が成立する。例として、買方 `B` が売方 `S` から `COMM:good.food` を 10 単位、価格 5 (`CUR:ALD` 最小単位/食料1単位) で取得する約定を示す。手数料は存在せず、約定は買方 −cash / 売方 +cash の純粋な現物授受である。
...
`cash = 5×10 = 50`。`CUR:ALD` の借方合計 (B:50) と貸方合計 (S:50) は一致し総量保存。`good.food` も借方=貸方=10 で保存。
...
```
資産時価 Assets(E) = Σ_a  balance[E][a] × mark_price(a)        # WUI 建て (信用ポジションの担保時価を含む)
                    + Σ_p  pool_share_claim(E, p)             # 貸借プール / AMM プールの持分請求権 (WUI 建て)
負債 Liabilities(E) = Σ_b  outstanding_face(E, b)              # 発行体側の BOND/BILL 額面 (WUI 建て)
                    + Σ_k  position_debt(E, k)                # 信用ポジション債務 = borrowed_value + accrued_interest (WUI 建て)
NetWorth(E)        = Assets(E) − Liabilities(E)
```

- `mark_price(a)`: 資産 `a` の WUI 建て単価。`CUR:xx` は FX を介して WUI 換算。`COMM`/`EQ`/`BOND` は直近約定価格を建値通貨で取り、FX で WUI 換算。直近約定が無い資産は構成の参照価格 ([16](16-configuration-and-initialization.md)) または直前 tick の価格を用いる。信用ポジションの担保アセット (ロングは買った `base`、ショートは得た `quote` + 自己証拠金) はその時価で `Assets(E)` に計上される。
...
- `position_debt(E, k)`: `E` が建てた信用ポジション `k` の返済債務であり、`borrowed_value + accrued_interest` (WUI 換算) で評価する。`borrowed_value` はロングで `borrowed_qty` (借りた `quote`)、ショートで `borrowed_qty × mark` (借りた `base` の時価)。これは従来の `margin_owed` を `Position` (信用ポジション) として具体化したものであり、現物の空売り禁止 (8.5) と整合する。プールが貸し出した現物アセット (`LOAN`) の対向として、プール側に同額の貸付債権 (持分請求権 = `Assets`)、借り手側に同額の返済債務 (`Liabilities`) が立つため、系全体では両者がネットして純資産は保存される (用語集 [00 §0.17](00-glossary.md))。
- `pool_share_claim(E, p)`: `E` が保有する貸借プール (`POOL:<asset>`) / AMM プール (`AMM:<pair>`) の持分が請求できるプール時価の按分。供給者の預入アセット (`POOL_SUPPLY`/`AMM_SUPPLY`) と分配利息・準備金成長 (供給金利・気配幅収益) を、持分比で評価する ([09 §貸借プール / §9.7](09-markets-and-trading.md))。供給者が放出した現物 (プール残高) と借り手の返済債務がネットするため、貸付はミントを伴わず保存則を破らない。
...
```mermaid
flowchart LR
  BAL["balance[E][a] (担保時価含む)"] --> MK["× mark_price(a) → WUI"]
  MK --> ASS["Assets(E)"]
  SHARE["プール持分請求権 (貸借 / AMM)"] --> ASS
  LIA["outstanding_face + Σ position_debt (borrowed_value + accrued_interest)"] --> LB["Liabilities(E)"]
...
労働市場で労働者 `W` が `COMM:labor.factory` を 1 単位、企業 `F` へ価格 30 (`CUR:ALD`/労働1単位) で販売する1約定の仕訳。手数料は存在せず、`cash = 30×1 = 30` が買方 `F` から売方 `W` へそのまま移転する。
...
- 板寄せ清算・注文種別・信用取引・貸借プール・強制決済・AMM・FX ペア: [09 市場と取引](09-markets-and-trading.md)
