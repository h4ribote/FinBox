...
各国の政策は、その国に配属された政治家エージェント集団の判断を以下の規則で集約して決定する (P3, `12-politics-and-government.md`)。各政治家の提案は `vote_weight` (既定 1.0、全員等重み) で加重する。政治家集合 `V = {1..n}`、政治家 `i` の重み `w_i`、`W = Σ w_i`。等重み (既定) では下記の加重平均/加重合計は単純平均/単純合計に一致する。

- **SCALAR (連続値)**: 最終値 = 提案値の**加重平均** `(Σ_i w_i·x_i)/W`。政策レンジでクランプし tick へ丸める。例 (等重み): 政策金利を A が 100bps、B が 200bps と判断 → 150bps (1.5%)。
- **BINARY (Yes/No)**: 各政治家は `[0,1]` の値で判断。**加重平均** `(Σ_i w_i·b_i)/W` が `≥ 0.5` なら Yes、未満なら No。
- **CATEGORICAL (複数選択肢)**: 各政治家が各選択肢をスコアリング。選択肢ごとの**加重合計スコア** `Σ_i w_i·s_i(c)` が最大の選択肢を採用 (同点は最小インデックス)。
- **ALLOCATION (配分ベクトル)**: 予算配分・軍事目標優先度などは、各政治家の L1 正規化済み配分ベクトルを `vote_weight` で**加重平均**し、再正規化して採用。
