...
- **クーポン支払 (P7)**: 利付国債は四半期境界 (`tick % (TURNS_PER_MONTH×3) == 0`、既定 12 ターン) ごとに、保有口数 `q` に対し `coupon = floor( q × face × coupon_rate / 10000 / 4 )` をプロトコル移転で支払う (10 §10.8.4・15 §15.8 と同一表現。四半期一括が正準、年率の四半期按分=単利、用語集 0.7。`/4` は1年=4四半期。丸めは `floor` で用語集 0.20 に従い、按分残差は発行体側で吸収)。支払元は `GOV:s`、支払先は当該銘柄の保有者全員 (台帳の保有スナップショットを P7 開始時点で確定)。
...
このターン按分式はファシリティ (11.3.4) および借入利息の毎ターン発生にのみ用いる。利付債のクーポンはこれを用いず、四半期一括 `floor( q × face × coupon_rate / 10000 / 4 )` が正準 (11.4.3、四半期境界でのみ発生)。両者は用途が異なるため混用しない。すべての発生額は台帳反映前に整数へ丸める (丸め方向は用語集 0.20 に従い `floor`)。
