...
通貨は用語集 0.6 の6種 (`CUR:ALD`, `CUR:BOR`, `CUR:CYR`, `CUR:DOR`, `CUR:ESM`, `CUR:FAR`) に固定する。各通貨の残高は整数で、その整数の1単位が**最小通貨単位** (minor unit) である。表示上の「1通貨」は `unit_scale` 最小単位に等しい (既定 `unit_scale = 100`、すなわち小数第2位相当を整数で保持)。価格・約定・利息はすべて最小単位の整数で行い、表示時のみ `unit_scale` で除して人間可読化する。`unit_scale` は通貨ごとに構成可 ([16](16-configuration-and-initialization.md))。
...
- **準備預金/常設ファシリティ**: 銀行モデル無効の簡易モードでは、すべてのエンティティが中央銀行に対し政策金利で1ターンの現金を預入/借入できる**常設ファシリティ**を持つと近似する。ターン末に余剰現金を預ければ `+policy_rate_turn`、不足を借りれば `-policy_rate_turn` の利息が P7 で発生する (借入は金融負債として計上、空売り同様に現物残高は負にしない、用語集 0.9)。このファシリティ利息は資金の機会費用を市場価格へ織り込ませる装置であり、既定では `RESERVE_FACILITY_ENABLED = true`。
- **物価への波及 (伝達経路)**: 政策金利の変化は次の経路で CPI・産出へ波及する。(1) 金利上昇 → 債券利回り上昇・割引現在価値低下 → 投資・企業の資金調達コスト増 → 生産投資抑制。(2) ファシリティ金利上昇 → 現金保有の機会費用増 → 消費需要の繰延べ → 財市場の需要曲線左シフト → 約定価格低下 → CPI 低下。波及は価格の内生的形成 (P4 板寄せ) を通じて自然に生じ、外生の価格操作は行わない。緩和は逆。
...
- **クーポン支払 (P7)**: 利付国債は四半期境界 (`tick % (TURNS_PER_MONTH×3) == 0`、既定 12 ターン) ごとに、保有口数 `q` に対し `coupon = round( q × face × coupon_rate / 10000 / 4 )` をプロトコル移転で支払う (10 §10.8.4 と同一表現。四半期一括が正準、年率の四半期按分=単利、用語集 0.7。`/4` は1年=4四半期)。支払元は `GOV:s`、支払先は当該銘柄の保有者全員 (台帳の保有スナップショットを P7 開始時点で確定)。
- **流通市場での売買 (P4)**: 発行後の `BOND`/`BILL` は二次市場で自由に売買され、価格・利回りは板寄せで内生的に変動する。空売りは金融負債計上で表現し現物残高は負にしない (用語集 0.9)。
...
P = Σ_{k=1..N} ( c / (1 + y·(t_k / TURNS_PER_YEAR)) ) + face / (1 + y·(T / TURNS_PER_YEAR))
  c   = face × coupon_rate / 10000 / 4        (四半期クーポン額, 整数丸め前)
...
```text
interest_turn = round( Pr × r_annual / 10000 / TURNS_PER_YEAR )      (ファシリティ/借入利息向け、毎ターン発生)
```

このターン按分式はファシリティ (11.3.4) および借入利息の毎ターン発生にのみ用いる。利付債のクーポンはこれを用いず、四半期一括 `round( q × face × coupon_rate / 10000 / 4 )` が正準 (11.4.3、四半期境界でのみ発生)。両者は用途が異なるため混用しない。すべての発生額は台帳反映前に整数へ丸める。
...
- 利息・クーポン・配当・割引: `round`(四捨五入、銀行丸めではなく算術丸め、`.5` は切り上げ) を用いる。
...
- **純資産 (NAV)**: エンティティの純資産は全ポジションを最新板寄せ清算価格でマークし WUI 換算した合計 (用語集 0.16):

```text
NAV_WUI(e) = Σ_assets balance[e][a] × mark_price(a) × fx_wui(ccy(a))
           − Σ_debts  outstanding_debt(e) × fx_wui(ccy(debt))
mark_price(CUR:s)        = 1 (その通貨建て), fx で WUI 換算
mark_price(COMM/BOND/EQ) = 直近板寄せ清算価格 (約定なき資産は前回値を持ち越し)
```

  債券・株式は時価、債務 (発行社債・ファシリティ借入) は控除。

### 11.9.3 投資家ランキング基準

- 投資家 (`INVESTOR`) およびプレイヤーの順位は `NAV_WUI` を正準基準とする (用語集 0.16)。リーダーボードはターン末 (P9) の `NAV_WUI` 降順。比較は通貨横断で一貫するよう必ず WUI 換算後の値で行う ([13](13-players-and-multiplayer.md))。
...
| パラメーター | 既定値 | 意味 |
| --- | --- | --- |
| `unit_scale` | 100 | 通貨の最小単位/表示単位比 (通貨別可) |
