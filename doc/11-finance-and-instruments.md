...
このターン按分式はファシリティ (11.3.4) および借入利息の毎ターン発生にのみ用いる。利付債のクーポンはこれを用いず、四半期一括 `floor( q × face × coupon_bps / 10000 / 4 )` が正準 (11.4.3、四半期境界でのみ発生)。両者は用途が異なるため混用しない。すべての発生額は台帳反映前に整数へ丸める (丸め方向は用語集 0.20 に従い `floor`)。
